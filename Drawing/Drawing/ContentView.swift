...
struct ContentView: View {
    ...
    var body: some View {
        Checkerboard(rows: rows, columns: columns)
            .onTapGesture {
                withAnimation {
                    self.rows = Int.random(in: 4...10)
                    self.columns = Int.random(in: 4...10)
                }
        }
    }
}
        ...
        ZStack {
            ForEach(0..<steps) { value in
                ...
                .inset(by: CGFloat(value))
                    .strokeBorder(LinearGradient(gradient: Gradient(colors: [self.getColor(for: value, brightness: 1), self.getColor(for: value, brightness: 0.5)]), startPoint: .top, endPoint: .bottom))
                    //.strokeBorder(self.getColor(for: value, brightness: 1), lineWidth: 2)
            }
        }
    .drawingGroup()
    }
    
    func getColor(for value: Int, brightness: Double) -> Color {
